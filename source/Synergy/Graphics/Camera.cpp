 ...
 */
Camera::Camera(float left, float right, float bottom, float top, float zNear,
		float zFar)
 ...
 */
Camera::Camera(float fieldOfViewY, float
aspectRatio, float zNear, float zFar)
    : Frustum(
		fieldOfViewY,  aspectRatio,
		zNear,  zFar),
 ...
 */
void Camera::
translateLocal(float left, float up, float forward) {
    eyePosition += left * l;
    eyePosition += up * u;
    eyePosition
		    += forward * f;
...
void Camera::lookAt(float centerX, float centerY, float centerZ) {
    ...
    l = cross(u, f);// If f and u are no longer orthogonal, make them so.
    if (dot(f, u) > 1e-7f) {
        u = cross(f,
		        l);
    }
